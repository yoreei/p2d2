    ...
    def visit_Assign(self, node):
        ...
        try:
            ...
            if (
                ...
                and type(node.value.value) == _ast.Attribute
                and node.value.value.attr == "loc"
                and type(node.value.slice.dims[0]) == _ast.Slice
                and node.value.slice.dims[0].lower
                == node.value.slice.dims[0].upper
                == node.value.slice.dims[0].step
                == None
                and type(node.value.slice.dims[1]) == _ast.Index
            ):
