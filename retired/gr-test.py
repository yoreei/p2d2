...
class CodeMatcher(unittest.TestCase):
    def matchSnipped(self, snipped, template, removeLinebreaks: bool = False):
        res, mapping, reason = self.doMatchSnipped(
            snipped.strip(), template.strip(), removeLinebreaks
        )
        if not res:
            mapstr = "with mapping:\n"
    ...
    def doMatchSnipped(self, snipped, template, removeLinebreaks):
        ...
        for p, o in zip(placeholders, occurences):
            if p not in mapping:
                ...
                return (
                    False,
                    mapping,
                    f"Mapping error: {p} -> {mapping[p]} exists, but {p} -> {o} found",
                )
        ...
        if len(placeholders) != len(occurences):
            return (
                False,
                mapping,
                f"number of placeholders {len(placeholders)} does not match occurences {len(occurences)}",
            )
        ...
        return (
            matches,
            mapping,
            "Snipped does not match template" if not matches else "",
        )
...
class DataFrameTest(CodeMatcher):
    ...
    def test_complexJoin(self):
        ...
        j = df1.join(
            df2, on=(df1["a"] == df2["b"]) & (df1["c"] <= df2["d"]), how="left outer"
        )
    ...
    def test_externaltable(self):
        ...
        try:
            df = grizzly.read_external_files(
                "filename.csv", ["a:int, b:str, c:float"], False
            )
            actual = df.generateQuery()
            expected = (
                "DROP TABLE IF EXISTS temp_ext_table$t0;"
            ...
            df = grizzly.read_external_files(
                "filename.csv", ["a:int, b:str, c:float"], True
            )
            actual = df.generateQuery()
            expected = (
                "DROP TABLE IF EXISTS temp_ext_table$t0;"
            ...
            df = grizzly.read_external_files(
                "filename.csv", ["a:int, b:str, c:float"], True, ",", "csv"
            )
            actual = df.generateQuery()
            expected = (
                "DROP TABLE IF EXISTS temp_ext_table$t0;"
