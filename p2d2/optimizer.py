...
class Ast2pr(ast.NodeTransformer):
    def is_proj(self, node):
        if (
            type(node.value) == _ast.Subscript
            and rgetattr(node, "value.value.attr") == "loc"
            and type(node.value.slice) == _ast.ExtSlice
        ):
            return True
        return False
    ...
    def is_sel(self, node):
        if (
            type(node.value) == _ast.Subscript
            and rgetattr(node, "value.value.attr") == "loc"
            and type(node.value.slice) == _ast.Index
            and type(node.value.slice.value) == _ast.Name
        ):
            return True
        return False
    ...
    def is_groupby_simple(self, node):
        if (
            type(node.value) == ast.Call
            and type(node.value.func) == ast.Attribute
            and node.value.func.attr in aggs.keys()
            and type(node.value.func.value) == ast.Call
            and type(node.value.func.value.func) == ast.Attribute
            and node.value.func.value.func.attr == "groupby"
        ):
            return True
        else:
            return False
    ...
    def is_simple_agg(self, node):
        if (
            type(node.value) == ast.Attribute
            and node.value.attr == "T"
            and type(node.value.value) == ast.Call
            and type(node.value.value.func) == ast.Attribute
            and node.value.value.func.attr == "to_frame"
            and type(node.value.value.func.value) == ast.Call
            and type(node.value.value.func.value.func) == ast.Attribute
            and node.value.value.func.value.func.attr in aggs.keys()
        ):
            return True
        else:
            return False
    ...
    def is_mode(self, node):
        if (
            type(node.value) == ast.Call
            and type(node.value.func) == ast.Attribute
            and node.value.func.attr == "head"
            and node.value.args[0].value == 1
            and type(node.value.func.value) == ast.Call
            and type(node.value.func.value.func) == ast.Attribute
            and node.value.func.value.func.attr == "mode"
        ):
            return True
        else:
            return False
    ...
    def fetch_colnames(self, name, lineno):
        ...
        cur.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_name='aggview'"
        )
    ...
    def visit_Assign(self, node):
        ...
        if rgetattr(node, "value.func.attr", None) == "read_sql_query":
            ...
            # TODO use addnode
            nodedict[tar] = {node.lineno: {"sql": query, "unresolved": []}}
            return
        ...
        if self.is_simple_agg(node):
            ...
            nodedict.addnode(
                tar,
                {
                    node.lineno: {
                        "sql": "SELECT "
                        + self.build_aggregate_list(agg_type, colnames)
                        + " FROM {}",
                        "unresolved": [src],
                    }
                },
        ...
        if self.is_sample(node):
            ...
            nodedict.addnode(
                ...
                {
                    node.lineno: {
                        "sql": "select * from {} order by random() "
                        + f"limit {nrows};",
                        "unresolved": [src],
                    }
                },
            ...
            nodedict.addnode(
                tar,
                {
                    node.lineno: {
                        ...
                        + ",".join(groupby_cols)
                        + ","
                        + self.build_aggregate_list(agg_type, agg_cols)
                        + " FROM {}\
                    GROUP BY "
                        + ",".join(groupby_cols)
                        + " ORDER BY "
...
def insert_pulls(tree):
    ...
    for node in tree.body:
        ...
        for name in reqs:
            ...
            if (name, lastdef_lineno) in pulled:
                continue
